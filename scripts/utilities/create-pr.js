...
function createPullRequest(runType, count) {
  ...
  if (runType === 'monthly_full') {
    title = `🔄 Monthly Full Refresh: ${count} high confidence trust centers`;
    body = `## 🔄 Monthly Full Refresh - Trust Center Discovery
...
### 🔍 What was verified:
- ✅ CNAME points to known trust center hosting platform
- ✅ Domain follows trust center patterns (trust.domain, etc.)
- ✅ Content contains trust center keywords
...
These entries have been automatically verified and are ready for immediate merge.`;
    ...
    title = `🤖 Weekly Discovery: ${count} high confidence trust centers`;
    body = `## 🎯 Weekly Trust Center Discovery
...
These entries have been automatically verified and are ready for immediate merge.`;
  ...
  try {
    ...
    const tempFile = '/tmp/pr-body.txt';
    fs.writeFileSync(tempFile, body);
    
    const prCommand = `gh pr create --title "${title}" --body-file "${tempFile}" --base main ${labelString}`;
